...
//prediction_obstacles 预测的障碍物 reference_line哪个道路参考线
DecisionData::DecisionData(
  ...
  for (const auto& prediction_obstacle :
    ...
    if (prediction_obstacle.trajectory().empty()) {//如果障碍物的轨迹是空
    ...
    for (const auto& trajectory : prediction_obstacle.trajectory()) {
      if (!IsValidTrajectory(trajectory)) {//判断轨迹是否合法
...
//创建虚拟障碍物
//point x y s heading v kappa dkappa 障碍物的ID
bool DecisionData::CreateVirtualObstacle(const ReferencePoint& point,
  ...
  common::math::Box2d box(box_center, heading, FLAGS_virtual_stop_wall_length,
                          lane_left_width + lane_right_width);//二维盒子类对象box
