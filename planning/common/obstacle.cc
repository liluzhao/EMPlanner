...
namespace {
const double kStBoundaryDeltaS = 0.2;        // meters ST边界s的差值阈值 0.2m
const double kStBoundarySparseDeltaS = 1.0;  // meters ST
const double kStBoundaryDeltaT = 0.05;       // seconds ST时间差值阈值
}  // namespace
...
Obstacle::Obstacle(const std::string& id,
  ...
  //如果障碍物多边形的点数小于2，就获取障碍物所有的定点。
  //定点是感知的长度 几何中心 heading计算来的
...
Obstacle::Obstacle(const std::string& id,
  ...
  trajectory_ = trajectory;//预测的轨迹
  auto& trajectory_points = *trajectory_.mutable_trajectory_point();
  double cumulative_s = 0.0;//累计纵向距离S
  ...
  for (int i = 1; i < trajectory_points.size(); ++i) {
    ...
    if (prev.relative_time() >= cur.relative_time()) {//检查轨迹点的时间信息
    ...
    trajectory_points[i].mutable_path_point()->set_s(cumulative_s);//累计距离
  }
}
//根据时间查询轨迹点
...
std::unique_ptr<Obstacle> Obstacle::CreateStaticVirtualObstacles(
  ...
  // create a "virtual" perception_obstacle 虚拟的感知障碍物
  perception::PerceptionObstacle perception_obstacle;
  // simulator needs a valid integer 合法的负ID
...
const STBoundary& Obstacle::path_st_boundary() const {
  return path_st_boundary_;//从障碍物的boundary;DP使用
}
...
void Obstacle::set_path_st_boundary(const STBoundary& boundary) {
  path_st_boundary_ = boundary;//从障碍物的boundary;DP使用；这里初始化；
  path_st_boundary_initialized_ = true;
}

void Obstacle::SetStBoundaryType(const STBoundary::BoundaryType type) {
  path_st_boundary_.SetBoundaryType(type);//从障碍物的boundary;DP使用；这里初始化类型；
}
