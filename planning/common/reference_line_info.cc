...
//adc_planning_point 自车规划起点   segments 车道段 reference_line参考线
...
//给定的S 找到相邻车道信息 ID 参考点的SL 车道宽 类型
...
//当前的referenceline的第一个点是否在上一个referenceline上面
...
bool ReferenceLineInfo::CombinePathAndSpeedProfile(
  ...
  // point for control module 密集采样0.02s 稀疏采样0.1 
...
// start 规划点是否用当前的位置还是用 当前时刻加0.1秒的规划周期
bool ReferenceLineInfo::AdjustTrajectoryWhichStartsFromCurrentPos(
  ...
  static constexpr double kMaxAngleDiff = M_PI_2;//90度
...
void ReferenceLineInfo::SetTurnSignalBasedOnLaneTurnType(
  ...
  // Set turn signal based on lane-change. 变道转向
  if (IsChangeLanePath()) {
  ...
  // Set turn signal based on lane-borrow. 借道转向
  ...
  // Set turn signal based on lane's turn type. 道路类型转向
  double route_s = 0.0;
