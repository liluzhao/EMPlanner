...
bool STBoundary::GetUnblockSRange(const double curr_time, double* s_upper,
                                  double* s_lower) const {//非阻塞障碍物的S范围
...
bool STBoundary::GetBoundarySRange(const double curr_time, double* s_upper,
                                   double* s_lower) const { //障碍物的ST Boundary的时间T对应S范围
  CHECK_NOTNULL(s_upper);
  CHECK_NOTNULL(s_lower);
  if (curr_time < min_t_ || curr_time > max_t_) {//
    return false;
  ...
  size_t left = 0;
  size_t right = 0;//
  if (!GetIndexRange(lower_points_, curr_time, &left, &right)) {//curr_time在障碍物的预测轨迹点vector(t,s)找到时间t，左右点，
    AERROR << "Fail to get index range.";
    return false;
  }
  const double r =
      (left == right
           ? 0.0
           : (curr_time - upper_points_[left].t()) /
                 (upper_points_[right].t() - upper_points_[left].t()));//lambda函数？？ upper_points_ 来源 线性比例
