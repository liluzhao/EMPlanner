...
//3. 平滑参考线的拼接
/*
平滑参考线拼接是针对不同时刻的RawReference，如果两条原始的RawReference是相连并且有覆盖的，
那么可以不需要重新去进行平滑，只要直接使用上时刻的平滑参考线，或者仅仅平滑部分anchor point即可。
例如上时刻得到的平滑参考线reference_prev，这时刻由RouteSegments得到的原始费平滑参考线
reference_current。由于RouteSegments生成有一个look_forward_distance的前向查询距离，
所以这时候车辆的位置很可能还在前一时刻的平滑参考线reference_prev，这时候就可以复用上时刻的
参考线信息，下面我们直接从代码来理解参考线拼接的流程和逻辑。
*/
bool ReferenceLineProvider::CreateReferenceLine(
  ...
  if (is_new_routing || !FLAGS_enable_reference_line_stitching) {
    ...
    for (auto iter = segments->begin(); iter != segments->end();) {
      reference_lines->emplace_back();
      if (!ExtendReferenceLine(vehicle_state, &(*iter), //ExtendReferenceLine参考线扩展/拼接函数
...
bool ReferenceLineProvider::ExtendReferenceLine(const VehicleState &state,
  ...
  /*
   2. 如果2种情况历史参考线遗留长度不够，那么久需要先对RouteSegments进行扩展，这部分在Pnc Map后接车道处理中有相关介绍。
   如果扩展失败直接进行平滑操作；如果扩展以后长度仍然不够，说明死路没有后继车道，只能复用历史平滑参考线。
  */
  double future_start_s =
      std::max(sl_point.s(), prev_segment_length -
                                 FLAGS_reference_line_stitch_overlap_distance);// 向后额外扩展look_forward_extend_distance的距离，默认50m
  double future_end_s =
  ...
  if (prev_segment->IsWaypointOnSegment(shifted_segments.LastWaypoint())) {
    *segments = *prev_segment;
    segments->SetProperties(segment_properties);
    *reference_line = *prev_ref;
    ADEBUG << "Could not further extend reference line";// C.2 扩展操作成功，但是扩招以后长度没有太大变化，死路，直接使用历史平滑参考线
    return true;
  }
  /*
  3. 如果3情况下扩展成功，并且额外增加了一定长度，得到了新的Path(也即新的RouteSegments)，接下来对新的路径进行平滑然后与历史平滑参考线进行拼接，
  就可以得到一条更长的平滑参考线。
  */
  ...
  if (!SmoothPrefixedReferenceLine(*prev_ref, new_ref, reference_line)) {
    ...
    return SmoothRouteSegment(*segments, reference_line);// SmoothPrefixedReferenceLine过程和普通的拼接其实没多大差异
  }
  if (!reference_line->Stitch(*prev_ref)) {
    AWARN << "Failed to stitch reference line";
    return SmoothRouteSegment(*segments, reference_line);// 两条平滑车道线拼接
  }
  if (!shifted_segments.Stitch(*prev_segment)) {
    AWARN << "Failed to stitch route segments";
    return SmoothRouteSegment(*segments, reference_line);// 两条平滑车道线对应的RouteSegments拼接
  }
  /*
   4. 当在4完成参考线的拼接以后，就可以得到一条更长的参考线，前向查询距离经过限制不会超出要求，但是随着车辆的前进，车后参考线的长度会变得很大，
   所以最后一步就是对车后的参考线进行收缩，保证车辆前后都有合理长度的参考线
  */
...
bool ReferenceLineProvider::Shrink(const common::SLPoint &sl,
  ...
  if (sl.s() > FLAGS_look_backward_distance * 1.5) {// 查询收缩下界
...
//GetAnchorPoint函数是如何完成采样点的坐标计算与轨迹点坐标矫正
/*当完成采样点的计算以后，下一步就是采样点(也就是轨迹点)的坐标矫正。为什么需要坐标矫正？
因为采样点坐标是在道路的中心线上，但当道路比较宽时，车辆不能一味的在中间行驶，需要考虑到
其他车辆超车情况。在这种情况下，车辆需要靠右行驶(当然不同区域的模式不一样，部分地区是靠
左形式)，所以道路过宽时，需要将轨迹点向右或者向左矫正一段距离。
*/
AnchorPoint ReferenceLineProvider::GetAnchorPoint(
 ...
 //1.计算车辆宽度adc_width和半宽adc_half_width
  ...
  //2计算车道距左边界距离left_width和距右边界距离right_width
...
//如果车道宽度大于车辆宽度的wide_lane_threshold_factor倍，默认为2，则需要靠边行驶，因为存在其他车辆超车的可能性
  ...
  double safe_lane_width = left_width + right_width;// 当前位置，车道总宽度
...
void ReferenceLineProvider::GetAnchorPoints(
    ...
    std::vector<AnchorPoint> *anchor_points) const {//路径的锚点
