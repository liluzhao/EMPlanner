...
//如果只有一条参考线（比如往某个方向只有一条车道），那就通过updatestatus将车辆状态设置为CHANGE_LANE_FINISHED，这也符合我们认知，单向只有一条车道，还换什么道，所以车辆就该一直处于换到结束的状态
  ...
  if (!has_change_lane) {
    ...
    return Status::OK();
  } else {  // has change lane in reference lines.//下面的else处理不止一条参考线的情况，正常道路都不止一条参考线，就连我们家小县城的马路都是单向3车道。主要逻辑为状态切换，实际操作还是通过updatestatus来实时更新车辆的换道状态。
...
//UpdateStatus()的操作就是通过lane_change_status指针来对车辆换道状态进行实时更新。
