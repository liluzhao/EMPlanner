...
Status SpeedBoundsDecider::Process(
  ...
  const PathData &path_data = reference_line_info->path_data();//横向规划的轨迹点
  const TrajectoryPoint &init_point = frame->PlanningStartPoint();//规划的起点
  const ReferenceLine &reference_line = reference_line_info->reference_line();
  PathDecision *const path_decision = reference_line_info->path_decision();//path decider
  ...
  for (auto *obstacle : path_decision->obstacles().Items()) {//遍历所有障碍物
    ...
    const auto &st_boundary = obstacle->path_st_boundary();//得到所有障碍物
    if (!st_boundary.IsEmpty()) {
      if (st_boundary.boundary_type() == STBoundary::BoundaryType::KEEP_CLEAR) {//判断停车区？？
