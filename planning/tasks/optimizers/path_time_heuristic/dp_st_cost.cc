...
DpStCost::DpStCost(const DpStSpeedOptimizerConfig& config, const double total_t,
  ...
  const auto dimension_t =
      ...
      1;//total_t 8s unit_t_ = 1s dimension_t = 9 ??
...
double DpStCost::GetObstacleCost(const StGraphPoint& st_graph_point) {
  ...
  if (FLAGS_use_st_drivable_boundary) {
    ...
    const double lower_bound =
        st_drivable_boundary_.st_boundary(index).s_lower();//可行驶区域 在st_boundary_decider设置
    const double upper_bound =
        st_drivable_boundary_.st_boundary(index).s_upper();

    if (s > upper_bound || s < lower_bound) {//可行驶区域之外无穷大
      return kInf;
    }
  }

  for (const auto* obstacle : obstacles_) { //多个obstacle
    ...
    auto boundary = obstacle->path_st_boundary();//从障碍物中拿到boundary；赋值的地址在
    //palnning->tasks->decisers->speed_bounds_deciser->st_boundary_mapper.cc  和palnning->tasks->decisers->st_bounds_deciser->st_obstacles_processor
    if (boundary.min_s() > FLAGS_speed_lon_decision_horizon) {
      continue;
    }
    if (t < boundary.min_t() || t > boundary.max_t()) {//ST图点的t必须要在boundary重叠
      continue;
    }
    if (boundary.IsPointInBoundary(st_graph_point.point())) {
      return kInf;//点在Boundary中
    }
    double s_upper = 0.0;
    double s_lower = 0.0;

    int boundary_index = boundary_map_[boundary.id()];//boundary_map_ 是一个无序map 键是string,值是int；boundary_cost_是对(pair)的多维数组
    if (boundary_cost_[boundary_index][st_graph_point.index_t()].first < 0.0) {
      boundary.GetBoundarySRange(t, &s_upper, &s_lower);//障碍物的s_lower s_upper缓存在boundary_cost_；在下一次求另外一个ST图点的障碍物代价时，直接取；
      boundary_cost_[boundary_index][st_graph_point.index_t()] =
          std::make_pair(s_upper, s_lower);//
    } else {
      s_upper = boundary_cost_[boundary_index][st_graph_point.index_t()].first;//上面缓存，这里直接取出。
