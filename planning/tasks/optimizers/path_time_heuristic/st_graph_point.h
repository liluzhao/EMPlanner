...
class StGraphPoint {
 ...
 private:
  STPoint point_;
  const StGraphPoint* pre_point_ = nullptr;//之前的ST图的点 常量指针 
  std::uint32_t index_s_ = 0;
  std::uint32_t index_t_ = 0;

  double optimal_speed_ = 0.0;//当前ST图点的速度
  double reference_cost_ = 0.0;//参考线代价
  double obstacle_cost_ = 0.0;//障碍物代价
  double spatial_potential_cost_ = 0.0;//受空间限制的代价
  double total_cost_ = std::numeric_limits<double>::infinity();//总代价初始无穷大
};
